...
get_cost <- function(tbbl){
  tbbl|>
    left_join(census_2021$all_noc_names, by=c("from"="noc_4"))|>
    rename(from_name=desc)|>
    left_join(census_2021$all_noc_names, by=c("to"="noc_4"))|>
    rename(to_name=desc)|>
...
make_segment_data <- function(transitions, coordinates){
  transitions|>
    left_join(coordinates, by = c("from" = "noc_4")) %>%
    rename(x_from = V1, y_from = V2) %>%
    left_join(coordinates, by = c("to" = "noc_4")) %>%
...
source_dest_plot <- function(source, dest, age_2011, age_2021, cut_off){
 # browser()
  full_join(source, dest, by = join_by(noc_4))|>
    mutate(diff=prop.y-prop.x,
           fill=if_else(diff<0, "Decreases", "Increases"))|>
    unite(noc_4, noc_4, desc, sep = ": ")|>
    filter(abs(diff)>cut_off)|>
    ggplot(aes(diff, fct_reorder(noc_4, diff, .desc = TRUE), fill=fill))+
    geom_col()+
    theme_minimal()+
...
census_2021$implicit_missing<- vroom(here("data", "9810059301_minor_canada.csv"))|>
  clean_names()|>
  rename(noc_desc=contains("noc"),
         age_2021=contains("age"))|>
  mutate(noc_4=str_sub(noc_desc, 1, 4),
         desc=str_sub(noc_desc, 6),
...
census_2021$all_noc_names <- census_2021$implicit_missing|>
  select(noc_4, desc)|>
  unique() #should be length 162

census_2021$noc_4 <- unique(census_2021$implicit_missing$noc_4)
census_2021$age_2021 <- unique(census_2021$implicit_missing$age_2021)
census_2021$all_2021_combos <- crossing(noc_4=census_2021$noc_4, age_2021=census_2021$age_2021)
...
census_2011$census_2011_raw <- vroom(here("data", "99-012-X2011033_nhs_canada.csv"))|>
  pivot_longer(cols=-`Occupation - Na`, names_to = "age_2011", values_to = "count")|>
  rename(noc_2016=`Occupation - Na`)|>
  filter(str_detect(noc_2016, "\\b\\d{3}\\b"),
         !str_detect(noc_2016, "Total"))|>
  mutate(noc_2016=str_sub(noc_2016, 1, 3))

#need to map to noc_4

census_2011$mapping <- read_csv(here("data","mapping", "noc2016v1_3-noc2021v1_0-eng.csv"))|>
  clean_names()|>
  select(noc_2016=noc_2016_v1_3_code, noc_2021=noc_2021_v1_0_code)|>
  mutate(noc_2016=str_sub(noc_2016,1,3),
         noc_2021=str_sub(noc_2021,1,4)
  )|>
  distinct()|>
  group_by(noc_2016)|>
  mutate(weight=1/n(), .after=noc_2016)
...
census_2011$census_2011 <- full_join(census_2011$census_2011_raw, census_2011$mapping, by="noc_2016")|>
  ...
  group_by(noc_4=noc_2021, age_2011)|>
  summarize(count=sum(weighted_value))|>
  mutate(age_2011=str_replace_all(age_2011," ","_"))|>
...
skills$onet_raw <- tibble(file=c("Skills.xlsx", "Abilities.xlsx", "Knowledge.xlsx", "Work Activities.xlsx"))%>%
  ...
  mutate(noc_4=str_sub(noc_5, 1,4), .after=noc_5)
...
skills$two_digit_skills <- anti_join(census_2021$all_noc_names, skills$onet_raw|>select(noc_4))|>
  select(noc_4)|>
  mutate(noc_2=str_sub(noc_4, 1,2))|>
  inner_join(skills$all_two_digit_skills)|>
  select(-noc_2)

skills$onet_full <- bind_rows(skills$onet_mapped, skills$two_digit_skills)|>
  column_to_rownames("noc_4")

skills$onet_pca <- prcomp(skills$onet_full, center=TRUE, scale=TRUE)
factoextra::fviz_eig(skills$onet_pca)
...
results <- bind_cols(census_2011$census_2011, census_2021$census_2021)|>
  mutate(facet_label=paste(age_2011,age_2021, sep=" -> "),
         ...
         props_2011_vec = map(props_2011, \(tbbl) tbbl |> deframe()),
...
write_rds(skills, "skills_canada.rds")
write_rds(results, "results_canada.rds")
